...
def select_species(source_df: pd.DataFrame) -> pd.DataFrame:
    ...
    selected_species_df = source_df[(
        source_df["variety"].isin(selected_species))]
    if selected_species:
...
def show_scatter_plot(selected_species_df: pd.DataFrame):
    ...
    feature_x = st.selectbox("Which feature on x?",
                             selected_species_df.columns[0:4])
    feature_y = st.selectbox("Which feature on y?",
                             selected_species_df.columns[0:4])

    fig = px.scatter(selected_species_df, x=feature_x,
                     y=feature_y, color="variety")
    st.plotly_chart(fig)
...
def show_histogram_plot(selected_species_df: pd.DataFrame):
    ...
    fig2 = px.histogram(selected_species_df, x=feature,
                        color="variety", marginal="rug")
